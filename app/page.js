...
import { saveAs } from 'file-saver';  // For data export
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user) {
        setUser(user);
        updateInventory();  // Load inventory for the authenticated user
      } else {
        setUser(null);
        setInventory([]);
      }
  ...
  const exportData = () => {
    const csv = inventory.map(({ name, quantity, category }) => `${name},${quantity},${category}`).join('\n');
